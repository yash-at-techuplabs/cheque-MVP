...
import { Upload, FileDown, XCircle, FileText, Copy, Check } from "lucide-react";
...
export default function Page() {
  ...
  const handleExtract = async () => {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `Server returned non-JSON (HTTP ${response.status}): ${raw.slice(0, 200)}`
        );
      }
  ...
  const ResultCard = ({ result, highlight = false }: { result: ExtractionResult | null, highlight?: boolean }) => {
              ...
              <AnimatePresence>
                {results.lite && (
                  <ResultCard
                    ...
                    highlight={
                      !results.flash &&
                      results.lite.success &&
                      (results.lite.data?.confidenceScore ?? 0) >= 90
                    }
                  />
                )}
                {results.flash && (
